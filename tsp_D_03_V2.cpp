...
#include <iomanip>
using namespace std;
// **********************   Versão com geração randomica de valores **
...
// Versão V2 para multiplos voos cobrindo todos os pontos.
//   versao tsp_D_03_V2  com X de 0 a 900   e Y de 0 a 400 
...
int main(int argc, char *argv[])
{
	...
	int n=10 ;  //  numero de nós na simulação 
	int Deq=600, Dmax=9000;
	time_t rawtime;
	int i, j, k=0, ic=0 , iv=0, im, totdis=0, tp=0, ic2=0 , ndrone=1;
	float maior=0.0;
	int totdv=0;
	...
	// Gerando os valores aleatorios de X Y e Ps
	...
	ps[0]=0;
	for (i=1; i<n;i++){
		k=rand() % 900;  // limite maximo de X 900
		xs[i]=k;
		k=rand() % 400;   // limite maximo de Y 400
 		ys[i]=k;
		//   k=rand() % 20;    //  peso limite maximo 20
		ps[i]=1;
	}
	...
	visitado[0]=1;
	caminho[0]=0;
	cout << "\nOrigem \tDestino    Distancia \tDistancia \tDistancia \tDistancia ";
	...
	while (ic < n){
		...
		if ((totdis+totdv) < Dmax ) {
			...
			//  imprime caminho
			cout << "\n\n Caminho do Drone numero " << ndrone  << " = ";
			for(i=0;i<=tp;i++){ cout << " " << caminho[i]; caminho [i] =0; 	}
			cout << "\n\n";
			i=0;
			ic--;
			totdis=0;
